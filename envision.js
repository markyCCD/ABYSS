	...
	ApImageZoom.prototype = {
		...
		_bind: function() {
			...
			if (this.settings.mouseWheelPluginEnabled && typeof jQuery.fn.mousewheel == "function") {
				this.$overlay.mousewheel(function(evt, delta, deltaX, deltaY) {
					self._onMouseWheel(evt, delta, deltaX, deltaY);
					return false;
				});
			}
		},
		...
		_onMouseWheel: function(evt, delta, deltaX, deltaY) {
			...
			var zoom = this.currentZoom + (this.settings.zoomStep * deltaY * -1);
			var origin = this._getPointerOrigin(evt)
